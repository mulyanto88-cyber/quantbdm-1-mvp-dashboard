...
export default function StockDetailPage() {
  ...
  const [ownershipDetails, setOwnershipDetails] = useState<OwnershipDetail[]>([])
  ...
  // Ownership pie grouping by investor_type
  ...
  // Ownership detail table
  const ownershipDetailTable = useMemo(() => {
    if (!ownershipDetails.length) return []
    return ownershipDetails.map(d => ({
      ...d,
      localForeign: d.local_foreign === 'F' ? 'Foreign' : 'Local',
    }))
  }, [ownershipDetails])
  ...
  return (
    <div className="space-y-4 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass rounded-xl p-4 border border-white/[0.06]">
          <div className="flex items-center justify-between mb-3">
            ...
            ><ExternalLink className="w-3 h-3"/> Full Summary</Link>
          </div>
          {brokerData.length > 0 ? (
            <div className="space-y-1.5">
      ...
      {/* ════════════════════════════════════════════════════════════
          OWNERSHIP STRUCTURE (PIE CHART + TABLE)
      ════════════════════════════════════════════════════════════ */}
                  ...
                  <PieChart>
                    <Pie data={ownershipPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} stroke="none">
                      {ownershipPieData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={INVESTOR_TYPE_COLORS[entry.name] || '#6b7280'} />
                      ))}
                    </Pie>
                    <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', fontSize: '11px' }} formatter={(value: any, name: any) => [`${value.toFixed(1)}%`, name]} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
      ...
      {/* Tombol ke Broker Summary (tengah) */}
